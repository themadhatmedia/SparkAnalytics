...
use App\Mail\WeeklyAlert;
use Mail;
use Carbon\Carbon;
use App\Models\Utility;
class AutoWeeklyAlert extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'auto:weeklyaletr';
    ...
    public function handle()
    {
        ...
        $alerts = Alert::where('duration', "weekly")
                    ->get();
                    ...
                    foreach ($user as  $val) {
                        ...
                        if($alt->email_notification==1)
                        {
                           ...
                           try{
                                ...
                                Mail::to($val->email)->send(new WeeklyAlert($alt,$val));
                            }catch(\Throwable $e){
                                Log::info($e);
                            }
                        ...
                        if($alt->slack_notification==1)
                        {
                            try{
                                ...
                                $data=app('App\Http\Controllers\Controller')->week_alter_data($alt->site_id,$alt->metric);
