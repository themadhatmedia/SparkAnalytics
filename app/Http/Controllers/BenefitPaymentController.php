...
class BenefitPaymentController extends Controller
{
    public function initiatePayment(Request $request)
    {
        ...
        if ($plan) {
            $get_amount =  (float)$plan->{$request->benefit_payment_frequency . '_price'};
            try {
                if (!empty($request->coupon)) {
                    ...
                    if (!empty($coupons)) {
                        ...
                        if ($get_amount <= 0) {
                            ...
                            $authuser->save();
                            $assignPlan = $this->assignPlan($plan->id ,$authuser->id,$request->benefit_payment_frequency);
                ...
                $userData =
                    [
                        ...
                        "redirect" => ["url" => route('benefit.call_back', ['plan_id' => $plan->id, 'amount' => $get_amount, 'coupon' => $coupon , 'frequency' => $request->benefit_payment_frequency ])],
