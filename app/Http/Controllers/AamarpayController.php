...
class AamarpayController extends Controller
{
    ...
    public function pay(Request $request)
    {
        ...
        if ($plan) {
            $get_amount =  (float)$plan->{$request->aamarpay_payment_frequency . '_price'};
            ...
            try {
                if (!empty($request->coupon)) {
                    ...
                    if (!empty($coupons)) {
                        ...
                        if ($get_amount <= 0) {
                            ...
                            $authuser->save();
                            $assignPlan = $this->assignPlan($plan->id ,$authuser->id,$request->aamarpay_payment_frequency);
                ...
                $fields = array(
                    ...
                    'success_url' => route('pay.aamarpay.success', Crypt::encrypt(['response'=>'success','coupon' => $coupon, 'plan_id' => $plan->id, 'price' => $get_amount, 'order_id' => $orderID , 'frequency' => $request->aamarpay_payment_frequency])),
                    //your success route
                    'fail_url' => route('pay.aamarpay.success', Crypt::encrypt(['response'=>'failure','coupon' => $coupon, 'plan_id' => $plan->id, 'price' => $get_amount, 'order_id' => $orderID , 'frequency' => $request->aamarpay_payment_frequency])),
                    //your fail route
    ...
    public function aamarpaysuccess($data, Request $request)
    {
        ...
        if ($data['response'] == "success")
        {
            ...
            $assignPlan = $this->assignPlan($plan->id ,$user->id, $data['frequency']);
        ...
        {
            return redirect()->route('plans')->with('error', __('Your payment is cancel'));
        }
        else {
