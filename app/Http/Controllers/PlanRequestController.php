...
class PlanRequestController extends Controller
{
    ...
    {
        if (\Auth::user()->can('manage request plan')) {
            $data = Auth::user();
            if (\Auth::user()->user_type == 'super admin') {
                $plan_requests = PlanRequest::get();

                return view('admin.plan.plan-request')->with('plan_requests', $plan_requests);
            }
        } else {
            return redirect()->back()->with('error', __('Permission Denied.'));
        }
    }
    public function userRequest($plan_id, $frequency = '')
    {
        // $frequency = $request->query('frequency');
        $objUser = Auth::user();
        if (\Auth::user()->can('send plan request')) {
    ...
    {
        if (Auth::user()->user_type == 'super admin' && \Auth::user()->can('approve plan request')) {
            ...
            if (!empty($plan_request)) {
                ...
                if ($response == 1) {
                    ...
                    $assignPlan = $this->assignPlan($plan_request->plan_id, $plan_request->user_id, $plan_request->duration);
