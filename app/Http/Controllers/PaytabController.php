...
class PaytabController extends Controller
{
    ...
    public function planPayWithpaytab(Request $request)
    {
        try {
            ...
            $user = Auth::user();
            if ($plan) {
                $get_amount =  (float)$plan->{$request->paytab_payment_frequency . '_price'};
                ...
                if (!empty($request->coupon)) {
                    ...
                    if (!empty($coupons)) {
                        ...
                        if ($get_amount <= 0) {
                            ...
                            $authuser->save();
                            $assignPlan = $this->assignPlan($plan->id ,$authuser->id,$request->paytab_payment_frequency);
    ...
    public function PaytabGetPayment(Request $request)
    {
        ...
        try {
            // if ($request->respMessage == "Authorised") {
                ...
                $assignPlan = $this->assignPlan($plan->id ,$user->id,$request->data['paytab_payment_frequency']);
