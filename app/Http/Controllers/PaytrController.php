...
class PaytrController extends Controller
{
    public function PlanpayWithPaytr(Request $request)
    {
        ...
        $currency =!empty(env('CURRENCY')) ? env('CURRENCY') : '';
        $planID = \Illuminate\Support\Facades\Crypt::decrypt($request->plan_id);
        $authuser = Auth::user();
        $plan = Plan::find($planID);

        if ($currency !== 'TL') {
            return redirect()->back()->with('error', __('Transaction currency must be TL.'));
        }

        if ($plan) {

            $get_amount =  (float)$plan->{$request->paytr_payment_frequency . '_price'};
                ...
                if (!empty($coupons)) {
                    ...
                    if ($get_amount <= 0) {
                        ...
                        if (!empty($plan)) {
                            ...
                            $assignPlan = $this->assignPlan($plan->id , $authuser->id , $request->paytr_payment_frequency);
            ...
            try {
                ...
                if ($result['status'] == 'success') {
                    ...
                    return redirect()->route('plans')->with('error', $result['reason']);
                }
                return view('admin.plan.paytr_payment', compact('token'));
            } catch (\Throwable $th) {
    ...
    public function paytrsuccess(Request $request)
    {
        if ($request->payment_status == "success") {
            try {
                ...
                $order->save();
                $assignPlan = $this->assignPlan($plan->id , $user->id , $request->paytr_payment_frequency);
