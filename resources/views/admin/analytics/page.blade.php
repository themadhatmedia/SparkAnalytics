    ...
    @if (count($site_data) > 0)
        <div class=" d-flex align-items-center flex-wrap gap-3">
            <div class="select-box">
                <select class="form-select" name="site_name" id="site-list">
            ...
            @if (\Auth::user()->can('manage share report settings'))
                <div class="btn p-0">
                    <a class="btn btn-primary" onclick="share_setting('page');" data-bs-toggle="modal"
                        data-bs-target="#share_page_report">
                        <span><i class="ti ti-settings text-white" data-bs-toggle="tooltip"
                                data-bs-original-title="{{ __('Share Report Setting') }}"></i>
                        </span></a>
                </div>
            @endif
            ...
            @endif
            <a href="#" class="btn btn-primary" onclick="saveAsPDF('page page')"data-bs-toggle="tooltip"
...
@section('content')
                        ...
                        <div class="row">
                                    ...
                                    <ul class="nav nav-pills nav-fill row" id="pills-tab" role="tablist">
                                        ...
                                        @foreach ($dimension_option as $key => $dimension)
                                            <li class="nav-item col-sm-1" role="presentation">
                                                <button class="nav-link page-analytics {{ $i == 0 ? ' active' : '' }} "
                                                    id="pills-tab-1" onclick="get_page_data()" data-bs-toggle="pill"
                                                    data-value="{{ $key }}" data-bs-target="#"
                                                    type="button">{{ $dimension }}</button>
                            ...
                            <div class="col-md-2">
                                <div class="input-group mr-sm-2">
                                    <input type="text" name="date_duration" onchange="get_page_data()"
                                        class="form-control date_duration w-100" id="date_duration" />
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card-body">
                        <div class="tab-content" id="pills-tabContent">
                            ...
                            @foreach ($metric_option as $key => $val)
                                @if ($j == 1)
                                    <div class="tab-pane fade show active" id="channel-chart-{{ $j }}"
            ...
            @if (count($metric_option) > 0)
                ...
                @foreach ($metric_option as $key => $val)
                    <li class="nav-item col-md-3" role="presentation">
                        @if ($i == 1)
                            <div class="card nav-link page-card-metrics active" id="{{ $key }}"
                                data-bs-toggle="pill" data-bs-target="#channel-chart-{{ $i }}" type="button">
                            @else
                                <div class="card nav-link page-card-metrics" id="{{ $key }}"
                                    data-bs-toggle="pill" data-bs-target="#channel-chart-{{ $i }}"
                                    data-id="{{ $key }}" type="button">
                        @endif
    ...
    <div id="share_page_report" class="modal fade" tabindex="-1" aria-labelledby="exampleModalPopoversLabel"
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <form method="POST" action="{{ route('save-share-setting', 'page') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <div class="form-check form-switch">
                                    <input onclick="password_status()" type="checkbox"
                                        class="form-check-input input-primary"name="is_password" id="is_password">
                                ...
                                <div style="display: none" id="password-box">
                                    <div class="action input-group input-group-merge  text-left ">
                                        <input type="password" value="12345678" class=" form-control " name="password"
                                            autocomplete="new-password" id="password" placeholder="Enter Your Password">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" title="{{ __('Copy') }}"
