...
<div class=" d-flex align-items-center flex-wrap gap-3">
   <div class="select-box">
    ...
    @endif
     <a href="#" class="btn btn-primary" onclick="saveAsPDF('channel page')"data-bs-toggle="tooltip" title="{{__('Download')}}" data-original-title="{{__('Download')}}">
                ...
                <ul class="nav nav-pills nav-fill row" id="pills-tab" role="tablist">
                    ...
                    @foreach($dimension_option as $key => $dimension)
                      <li class="nav-item col-sm-1" role="presentation">
                        <button class="nav-link chanel-analytics {{ ($i == 0) ? ' active' : '' }} " id="pills-tab-{{$i}}" id="pills-tab-3" data-value="{{$key}}"  onclick="get_channel_data()" data-bs-toggle="pill" data-bs-toggle="pill"
  ...
  <div class="card">
    <div class="card-body">
      <div class="tab-content" id="pills-tabContent">
        ...
        @if($j==1)
          <div class="tab-pane fade show active" id="channel-chart-{{$j}}" role="tabpanel"aria-labelledby="pills-user-tab-1">
        @else
          <div class="tab-pane fade =" id="channel-chart-{{$j}}" role="tabpanel" aria-labelledby="pills-user-tab-1">
        @endif
          <div class="row">
            <div class="col-md-6">
               <div class="card">
                  <div class="card-body">
                    <div id="channel-line-chart-{{$key}}">
          ...
          <ul class="nav nav-pills nav-fill row" id="pills-tab" role="tablist">
            ...
            @foreach($metric_option as $key => $val)
              <li class="nav-item col-md-3" role="presentation">
                @if($i==1)
                <div class="card nav-link card-metrics active" id="{{$key}}" data-bs-toggle="pill"
                      data-bs-target="#channel-chart-{{$i}}"  type="button">
                @else
                <div class="card nav-link card-metrics" id="{{$key}}" data-bs-toggle="pill"
                      data-bs-target="#channel-chart-{{$i}}" data-id="{{$val}}" type="button">
                @endif
                  <div class="card-body px-lg-5">
                    <h4 id="metric_data_{{$key}}">0</h4>
                    <h5>{{$val}}</h5>
                  </div>
                </div>
