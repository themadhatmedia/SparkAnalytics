...
<head>
    <title>{{ $header_text ? $header_text : config('app.name', 'AnalyticsGo SaaS') }}</title>
    <!-- Meta -->
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=0, minimal-ui" />

    <meta name="title"
        content="{{ isset($setting['meta_keywords']) ? $setting['meta_keywords'] : config('app.name', 'CMMSGo SaaS') }}">
    <meta name="description"
        content="{{ isset($setting['meta_description']) ? $setting['meta_description'] : 'With Meta Tags you can edit and experiment with your content then preview how your webpage will look on Google, Facebook, Twitter and more!' }}">
    ...
    <meta property="og:image"
        content= "{{ $meta_image . (isset($meta_logo) && !empty($meta_logo) ? 'meta_image.png' : 'meta-image.png') }}">
    ...
    <meta property="twitter:description"
        ccontent="{{ isset($setting['meta_description']) ? $setting['meta_description'] : 'With Meta Tags you can edit and experiment with your content then preview how your webpage will look on Google, Facebook, Twitter and more!' }}">
            ...
            <nav class="navbar navbar-expand-md  default top-nav-collapse">
                ...
                <div class="collapse navbar-collapse" id="navbarTogglerDemo01">
                    <ul class="navbar-nav">
                        ...
                        <li class="nav-item {{ $settings['feature_status'] == 'on' ? '' : 'd-none' }}">
                        ...
                        <li class="nav-item {{ $settings['plan_status'] == 'on' ? '' : 'd-none' }}">
                            <a class="nav-link" href="#plan">{{ $settings['plan_title'] }}</a>
                        </li>
                        <li class="nav-item {{ $settings['faq_status'] == 'on' ? '' : 'd-none' }}">
    ...
    <section class="subscription bg-primary section-gap" id="plan">
        <div class="container">
            ...
            <div class="row justify-content-center">
                @php
                    $collection = \App\Models\Plan::take(4)
                        ...
                        <div
                            ...
                            <div class="card-body">
                                ...
                                <span class="mb-4 f-w-600 p-price">
                                    ...
                                    {{ $value->monthly_price }}
                                    <small class="text-sm">/{{ __('Month') }}</small>
                                </span>
                                ...
                                <ul class="list-unstyled my-3">
                                    <li>
                                        <div class="form-check text-start">
                                            <label class="form-check-label"
                                                for="customCheckc1">{{ $value->trial_days == -1 ? 'Unlimited' : $value->trial_days }}
                                                {{ __(' Trial Days') }}</label>
                                        </div>
                                    </li>
                                    <li>
                                        <div class="form-check text-start">
                                            <label class="form-check-label"
                                                for="customCheckc1">{{ $value->max_site == -1 ? 'Unlimited' : $value->max_site }}
                                                {{ __('Sites') }}</label>
                                        </div>
                                    </li>
                                    <li>
                                        <div class="form-check text-start">
                                            <label class="form-check-label"
                                                for="customCheckc1">{{ $value->max_widget == -1 ? 'Unlimited' : $value->max_widget }}
                                                {{ __('Widget Per Site') }}</label>
                                        </div>
                                    </li>
                                    <li>
                                        <div class="form-check text-start">
                                            <label class="form-check-label"
                                                for="customCheckc1">{{ $value->max_user == -1 ? 'Unlimited' : $value->max_user }}
                                                {{ __('Users') }}</label>
                                        </div>
                                    </li>
                                    <li>
                                    ...
                                    <li>
                                        <div class="form-check text-start">
                                            <label class="form-check-label"
                                                for="customCheckc1">{{ $value->analytics == 1 ? 'Enable' : 'Disable' }}
                                                {{ __('Analytics') }}</label>
                                        </div>
                                    </li>
                                </ul>
                                <div class="d-grid">
                                    <a href="{{ route('register', ['value' => Crypt::encrypt($value->id), 'trial' => true]) }}"
                                        class="btn btn-primary rounded-pill">{{ __('Start with
                                                                            Starter') }}
                                    ...
                                    <div class="card bg-dark shadow-none mb-0">
                                        <div class="card-body p-3">
                                            ...
                                            <div class="d-flex  align-items-center ">
                                                ...
                                                <img src="@if (!empty($value->testimonials_user_avtar)) {{ $logo . '/' . $value->testimonials_user_avtar }} @else {{ $logo . '/avatar.png' }} @endif"
...
<footer class="site-footer bg-gray-100">
        ...
        <div class="footer-row">
            ...
            <div class="ftr-col">
                <ul class="list-unstyled">
                    @if (is_array(json_decode($settings['menubar_page'])) || is_object(json_decode($settings['menubar_page'])))
                        @foreach (json_decode($settings['menubar_page']) as $key => $value)
                            @if (($value->header == 'on' && $value->footer == 'off') || $value->footer == 'on')
    ...
    <div class="border-top border-dark text-center p-2">
        <p class="mb-0"> @php $year=date('Y'); @endphp {{ '© ' . date('Y') }}
            {{ $setting['footer_text'] ? $setting['footer_text'] : config('app.name', 'AnalyticsGo SaaS') }}
        </p>
    </div>
</footer>
<!-- [ Footer ] end -->
